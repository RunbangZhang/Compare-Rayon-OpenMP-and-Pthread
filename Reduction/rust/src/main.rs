...
pub fn serial_reduction(do_square: bool, num_vec: &mut [u64]) -> u64 {
    if do_square == false {
        num_vec.iter().sum()
    } else {
...
// The parallel version of the reduction
pub fn par_reduction(do_square: bool, num_vec: &mut [u64]) -> u64 {
    if do_square == false {
