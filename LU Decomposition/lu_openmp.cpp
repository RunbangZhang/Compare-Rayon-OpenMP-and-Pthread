...
void LU_factorization_parallel(int n, float **A, float **L, float **U, int num_threads) {
    ...
    for (k = 0; k < n; k++) {
        ...
        #pragma omp parallel for shared(A, L, U, k) private(i) num_threads(num_threads) schedule(static)
        for (i = k + 1; i < n; i++) {
            L[i][k] = A[i][k] / U[k][k];
            U[k][i] = A[k][i];
        }

        #pragma omp parallel for shared(A, L, U, k) private(i, j) num_threads(num_threads) schedule(static)
        for (i = k + 1; i < n; i++) {
            for (j = k + 1; j < n; j++) {
...
int main(int argc, char *argv[]) {
    ...
    fp_L = fopen("output_omp_L.txt", "w");
    if (fp_L == NULL) {
        printf("Could not open file output_omp_L.txt for writing.\n");
        return 1;
    }
    ...
    fp_U = fopen("output_omp_U.txt", "w");
    if (fp_U == NULL) {
        printf("Could not open file output_omp_U.txt for writing.\n");
        return 1;
    }
